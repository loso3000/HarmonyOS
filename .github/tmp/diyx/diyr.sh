...
[ -f "include/kernel-6.12" ] && ver612=`grep "LINUX_VERSION-6.12"  include/kernel-6.12 | cut -d . -f 3`
[ -f "target/linux/generic/kernel-6.6" ] && ver66=`grep "LINUX_VERSION-6.6"  target/linux/generic/kernel-6.6 | cut -d . -f 3` |head -1
[ -f "target/linux/generic/kernel-6.12" ] &&  ver612=`grep "LINUX_VERSION-6.12"  target/linux/generic/kernel-6.12 | cut -d . -f 3` |head -1
date1="${DATA}_${CONFIG_S}-Sirpdboy"
if [ "$VER1" = "5.4" ]; then
