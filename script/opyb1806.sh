...
if [ "$VER2" = "5.4" ]; then
    ...
    sed -i 's/$(VERSION_DIST_SANITIZED)-$(IMG_PREFIX_VERNUM)$(IMG_PREFIX_VERCODE)$(IMG_PREFIX_EXTRA)/$(shell TZ=UTC-8 date +%Y%m%d -d +12hour)-dz-/g' include/image.mk
fi

echo "DISTRIB_REVISION='${date1} dz'" > ./package/base-files/files/etc/openwrt_release1
echo ${date1}' dz '>> ./package/base-files/files/etc/banner
