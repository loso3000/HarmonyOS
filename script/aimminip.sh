...
# date1='Ipv4-Bypass-Vip-R2023.01.01'
# sed -i 's/$(VERSION_DIST_SANITIZED)-$(IMG_PREFIX_VERNUM)$(IMG_PREFIX_VERCODE)$(IMG_PREFIX_EXTRA)/20230101-Ipv4-Bypass-Vip-5.4-/g' include/image.mk
if [ "$VER2" = "5.4" ]; then
    ...
    sed -i 's/$(VERSION_DIST_SANITIZED)-$(IMG_PREFIX_VERNUM)$(IMG_PREFIX_VERCODE)$(IMG_PREFIX_EXTRA)/$(shell TZ=UTC-8 date +%Y%m%d -d +12hour)-Ipv6-Bypass-Vip-6.0-/g' include/image.mk
fi
