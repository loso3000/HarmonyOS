...
# 修复 hostapd 报错
cp -f  ./package/other/patch/011-fix-mbo-modules-build.patch package/network/services/hostapd/patches/011-fix-mbo-modules-build.patch
# 取消主题默认设置
...
# date1='Ipv6-Mini-Vip-R2023.10.01'
# sed -i 's/$(VERSION_DIST_SANITIZED)-$(IMG_PREFIX_VERNUM)$(IMG_PREFIX_VERCODE)$(IMG_PREFIX_EXTRA)/20231001-Ipv6-Mini-VIP-6.1-/g' include/image.mk
if [ "$VER2" = "5.4" ]; then
...
echo "DISTRIB_REVISION='${date1} by Sirpdboy'" > ./package/base-files/files/etc/ezopenwrt_version
